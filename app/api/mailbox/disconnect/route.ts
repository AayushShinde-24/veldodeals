...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!context) return fail(new Error("Sign in before disconnecting a mailbox."), 401);
    const form = await request.formData().catch(() => null);
    const rawProvider = form?.get("provider") ?? request.nextUrl.searchParams.get("provider") ?? "google";
    const provider = rawProvider === "gmail" || rawProvider === "google_calendar" || rawProvider === "google" || rawProvider === "mailbox" ? rawProvider === "mailbox" ? "google" : rawProvider : "google";
    ...
    return fail(new Error("Mailbox disconnect failed."), 400);
  }
}
